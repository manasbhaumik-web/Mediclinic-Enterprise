...
import { 
  ...
  ShieldAlert, Zap, Globe, Award, HelpCircle, Mail, ChevronDown, MessageSquare, ArrowUp
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#f7fdfd] text-[#0f3c4c] font-sans selection:bg-[#0d9488] selection:text-white relative">
      ...
      <div className="bg-[#0f766e] text-teal-50 text-[11px] font-semibold py-1.5 px-4 flex items-center justify-between border-b border-[#0d9488] relative z-50">
        <div className="flex items-center gap-4 overflow-x-auto no-scrollbar whitespace-nowrap max-w-6xl mx-auto w-full justify-between">
          ...
          <div className="hidden md:flex items-center gap-6 text-[11px]">
            <span>🩺 Suite 101: <strong className="text-white">Dr. Sarah Jenkins</strong> (On Duty)</span>
            <span>⏱️ Avg Queue Wait: <strong className="text-white font-mono">11 mins</strong></span>
            <span>💊 Dispensary Stock: <strong className="text-emerald-300">100% Ready</strong></span>
            <span>💳 TPA Claim Gateway: <strong className="text-teal-200">Online 99.9%</strong></span>
          </div>
          <button 
            type="button" 
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {filteredDoctors.map((doc) => (
              <div key={doc.id} className="ice-mint-card-interactive p-6 text-center space-y-4 group relative">
                ...
                <div className={`w-20 h-20 rounded-2xl ${doc.bgColor} mx-auto flex items-center justify-center text-white text-2xl font-black shadow-md group-hover:scale-105 transition-transform`}>
                  {doc.id}
                </div>
      ...
      <div className="fixed bottom-6 right-6 z-40 flex flex-col gap-3 items-end">
        <button
          type="button"
          onClick={() => setIsBookingOpen(true)}
          className="px-4 py-3 rounded-2xl bg-[#0d9488] hover:bg-[#0f766e] text-white text-xs font-black shadow-2xl flex items-center gap-2 transition-all hover:scale-105 cursor-pointer ring-2 ring-white/80"
        >
          <Calendar className="w-4 h-4" />
          ...
          <div className="bg-[#f7fdfd] border border-[#ccfbf1] rounded-none max-w-lg w-full p-6 shadow-2xl space-y-5 relative text-[#0f3c4c]">
            ...
            {bookingSubmitted ? (
              <div className="py-6 text-center space-y-4">
                ...
                <div className="p-4 bg-[#f0fdfa] rounded-2xl border border-[#ccfbf1] text-left text-xs space-y-2">
