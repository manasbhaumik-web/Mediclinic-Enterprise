...
export default function StaffManagementModule() {
  ...
  const handleAddStaff = (e: React.FormEvent) => {
    ...
    const newStaff: StaffMember = {
      ...
      email: newEmail,
      status: 'Active',
      salaryBase: parseInt(newSalary) || 4000,
              ...
              {filteredStaff.length === 0 ? (
                ...
                <tr key={staff.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    ...
                    >
                      <div className={`w-1.5 h-1.5 rounded-full ${staff.status === 'Active' ? 'bg-emerald-500' : 'bg-red-500'}`}></div>
                      {staff.status}
                    </button>
                  </td>
                  <td className="px-6 py-4">
            ...
            <form onSubmit={handleAddStaff} className="p-5 space-y-4">
              ...
              <div className="space-y-1">
                ...
                <input 
                  ...
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-[#07B2B2] outline-none"
                  placeholder="jane.doe@mediclinic.local"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <select 
                    ...
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-[#07B2B2] outline-none bg-white"
                  >
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-600 uppercase">Base Salary (RM)</label>
                <input 
                  type="number" 
                  required
                  value={newSalary}
              ...
              <div className="pt-4 flex gap-3">
                <button 
                  type="button"
                  onClick={() => setIsAddModalOpen(false)}
                  className="flex-1 py-2.5 bg-slate-100 text-slate-600 hover:bg-slate-200 rounded-lg font-bold text-sm transition-colors cursor-pointer"
                >
                  Cancel
                </button>
                <button 
                  type="submit"
                  className="flex-1 py-2.5 bg-[#07B2B2] text-white hover:bg-[#058A8A] rounded-lg font-bold text-sm transition-colors cursor-pointer shadow-sm"
                >
                  Create Account
                </button>
              </div>
